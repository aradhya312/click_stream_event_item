...
object DataPipeline {
  def dataPipeline():Unit={
    ...
    val (df1rename,df2rename)=RenameColumn.renameColumn(df1duplicates,df2duplicates)
    val (df1lowercase,df2lowercase)=ConvertToLowercase.convertToLowercase(df1rename,df2rename)

    val joinedDF = FileWriter.fileWriter(df1lowercase,df2lowercase)
